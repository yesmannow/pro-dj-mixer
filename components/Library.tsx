              ...
              {history.length === 0 ? (
                ...
                <div className="divide-y divide-slate-800/50">
                  {history.map((item, idx) => {
                    const historyItemMatch = isSmartMatchEnabled && masterDeck.track && item.track
                    ...
                    <div key={`history-${item.id ?? idx}`} className="flex items-center justify-between px-4 py-3 bg-slate-900/30 group">
                      <div className="min-w-0 flex items-center gap-3">
                        ...
                        <div className={clsx("flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity", !historyItemMatch ? "opacity-0 pointer-events-none" : "")}>
                          <button
